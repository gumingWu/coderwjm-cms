...
import {
  genComponentsIndexTemplate,
  genComponentsVueTemplate
} from '../templates/createComponents'
import { MKDIR_FILE_OPTIONS, WRITE_FILE_OPTIONS } from '../shared/constant'

export default (type, name, path, useSetupName) => {
  ...
  switch (type) {
    case 'views':
      ...
      writeFileSync(
        filePath,
        genViewsTemplate(name, useSetupName),
        WRITE_FILE_OPTIONS
      )
      break
    case 'components':
      ...
      writeFileSync(
        filePath,
        genComponentsIndexTemplate(name),
        WRITE_FILE_OPTIONS
      )
      writeFileSync(
        cpnVueDir,
        genComponentsVueTemplate(name, useSetupName),
        WRITE_FILE_OPTIONS
      )
      break
    case 'router':
  ...
  logger.success(`The ${type}: ${name} has been cretaed successfully`)
  logger.info(`Target directory: ${filePath}`)
}
